...
async def subroutine_2s_complement_using_branch(dut):
    ...
    await clkedge
    dut._log.info(
        f"LDR R3, R0 => Load from memory location 0 to R3 <== 0x00000003 Array size"
    )
    print_all(dut)
    ...
    await clkedge
    dut._log.info(
        f"LDR R5, R0 #12 => Load from memory location 12 to R5 <== 0x00000010 Base Address"
    )
    print_all(dut)
    ...
    await clkedge
    dut._log.info(
        f"LDR R4, R5 #0 => Load from memory location 0 to R4 <== 0x00000005 Start of array address"
    )
    print_all(dut)
    ...
    for i in range(3):
        ...
        await clkedge
        dut._log.info(
            f"BEQ Branch to subroutine at address 0x00000004 if Z flag is set"
        )
        print_all(dut)
        ...
        if i == 0 or i == 1:
            ...
            await clkedge
            dut._log.info(
                f"Check PC value to see if it is 0x00000004 and result should be 193"
            )
            print_all(dut)
